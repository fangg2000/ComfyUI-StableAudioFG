...
def load_model(model_config=None, model_ckpt_path=None, pretrained_name=None, pretransform_ckpt_path=None, device="cuda", model_half=False):
    global sample_rate, sample_size, model_type
    cache_model = None
    
    if pretrained_name is not None:
        print(f"Loading pretrained model {pretrained_name}")
        cache_model, model_config = get_pretrained_model(pretrained_name)

    elif model_config is not None and model_ckpt_path is not None:
        print(f"Creating model from config")
        cache_model = create_model_from_config(model_config)

        print(f"Loading model checkpoint from {model_ckpt_path}")
        # Load checkpoint
        copy_state_dict(cache_model, load_ckpt_state_dict(model_ckpt_path))
    ...
    if pretransform_ckpt_path is not None:
        ...
        cache_model.pretransform.load_state_dict(load_ckpt_state_dict(pretransform_ckpt_path), strict=False)
        print(f"Done loading pretransform")

    cache_model.to(device).eval().requires_grad_(False)

    if model_half:
        cache_model.to(torch.float16)
        
    print(f"Done loading model")

    return cache_model, model_config
